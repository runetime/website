...
var AdminUserPanel = (function () {
    ...
    AdminUserPanel.prototype.addEvents = function () {
        $(this.hooks.listing.awardAdd).click(function (e) {
        ...
        $(this.hooks.listing.chatboxRemove).click(function (e) {
            var id = $(e.target).parent().parent().parent().attr('rt-data');
            var user = userPanel.getUsernameById(id);
            user.done(function (user) {
                user = $.parseJSON(user);
        ...
        $(this.hooks.listing.postsRemove).click(function (e) {
            var id = $(e.target).parent().parent().parent().attr('rt-data');
            var user = userPanel.getUsernameById(id);
            user.done(function (user) {
                user = $.parseJSON(user);
        ...
        $(this.hooks.listing.userBan).click(function (e) {
            var id = $(e.target).parent().parent().parent().attr('rt-data');
            var user = userPanel.getUsernameById(id);
            user.done(function (user) {
                user = $.parseJSON(user);
        ...
        $(this.hooks.listing.userBanPermanent).click(function (e) {
            $(userPanel.elements.modals.ipBan).modal('show');
        });
        $(this.hooks.listing.userMute).click(function (e) {
            var id = $(e.target).parent().parent().parent().attr('rt-data');
            var user = userPanel.getUsernameById(id);
            user.done(function (user) {
                user = $.parseJSON(user);
    ...
    AdminUserPanel.prototype.awardSubmit = function () {
        var id = $(this.elements.award.id).val(), username = $(this.elements.award.username).val();
    ...
    AdminUserPanel.prototype.banIpSubmit = function () {
        var address = $(this.elements.banIp.address).val(), reason = $(this.elements.banIp.reason).val();
        if (address.length <= 7) {
    ...
    AdminUserPanel.prototype.chatboxSubmit = function () {
        var username = $(this.elements.chatbox.username).val(), reason = $(this.elements.chatbox.reason).val();
        if (username.length === 0) {
    ...
    AdminUserPanel.prototype.done = function (message) {
    ...
    AdminUserPanel.prototype.error = function (message) {
        $.each(this.elements.modals, function (index, modal) {
            $(modal).modal('hide');
        });
        $(this.elements.modalResults.bad).modal('show');
        $(this.hooks.modals.results.bad).html(message);
    };
    AdminUserPanel.prototype.getAwards = function () {
    ...
    AdminUserPanel.prototype.getUsernameById = function (id) {
        var data = {
            id: id
        };
        return utilities.postAJAX(this.paths.username, data);
    };
    AdminUserPanel.prototype.muteSubmit = function () {
    ...
    AdminUserPanel.prototype.postsSubmit = function () {
        var username = $(this.elements.posts.username).val(), reason = $(this.elements.posts.reason).val();
        var data = {
            username: username,
            reason: reason
        };
    ...
    AdminUserPanel.prototype.search = function () {
    ...
    AdminUserPanel.prototype.searchAll = function () {
    ...
    AdminUserPanel.prototype.result = function (user) {
