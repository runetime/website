(function () {
  ...
  this.ban = function () {
    var ip = $(this.elements.ipBan.ip).val(),
        ...
        results = utilities.postAJAX(this.paths.ip.ban, data);
    ...
    results.done(function (results) {
      ...
      if (results.done === true) {
        this.done("The IP has been successfully banned.");
      } else {
        this.error("There was an unknown error while performing the IP ban.");
      }
    });
  };

  this.chatboxClear = function () {
    ...
    var self = this;
		...
		results.done(function(results) {
			...
			if(results.done === true) {
				this.done("The chatbox was successfully cleared.");
			} else {
				this.error("There was an unknown error while clearing the chatbox.");
			}
		});
  };
  ...
  this.radioStop = function () {
    ...
    results.done(function (results) {
      ...
      if (results.done === true) {
        self.done("The radio will successfully stop within 30 seconds.");
      } else {
        self.error("The radio has been switched on.");
      }
    });
  };

  this.staffDemote = function () {
    ...
    var self = this;
		results.done(function(results) {
			...
			if(results.done === true) {
				self.done("All staff were successfully demoted.");
			} else {
				self.error("There was an unknown error while demoting all staff.");
			}
		});
  };
}());
